...
function init(transer) {
  ...
  return {
    ...
    keyBinds: function (hero) {
      ...
      hero.addKeyBindFunc("BATTLE_CARD_RESET", (player, manager) => resetBattleCard(player, manager), "Return To Crimson Buster", 5);
    },
    isKeyBindEnabled: function (entity, keyBind) {
      var result = false;
      var uuid = "c4bc5db6-3cf6-44fe-8427-304a7b211bc4";
      if (keyBind == "PREDATION") {
        result = entity.getData("skyhighheroes:dyn/wave_changing_timer") == 1 && (entity.getData("skyhighheroes:dyn/battle_card") > 0 || entity.getData("skyhighheroes:dyn/crimson_timer") < 1);
      };
    ...
    isModifierEnabled: function (entity, modifier) {
      ...
      if (modifier.name() == "fiskheroes:transformation") {
        if (modifier.id() == "predation" || modifier.id() == "crimson") {
    ...
    waveHandler: function (entity, manager) {
      if (entity.getUUID() == "c4bc5db6-3cf6-44fe-8427-304a7b211bc4" && entity.world().isUnobstructed(entity.pos(), entity.pos().add(0,1000,0)) && entity.posY() >= 105 && entity.world().getLocation(entity.pos()).biome().startsWith("Extreme") && entity.world().isRaining() && entity.world().isThundering()) {
        var value = Math.random();
        if (value < 0.01) {
          manager.setData(entity, "skyhighheroes:dyn/em_being", "Crimson");
        ...
        if (entity.getData("skyhighheroes:dyn/calling_timer") == 1) {
          manager.setString(entity.getWornChestplate().nbt(), "HeroType", "skyhighheroes:lucas_stelar");
        };
      };
    }
  };
